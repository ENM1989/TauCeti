...
* `LieAlgebra.ad_pow_apply_eq`: the iterated adjoint action is the iterated bracket, so it does
  not depend on the ring of scalars the Lie algebra structure is taken over.
...
/-- **The iterated adjoint action does not depend on the ring of scalars.** A Lie ring that is a
Lie algebra over two rings has one bracket, and `ad` is that bracket, so `(ad x) ^ n y` is the
`n`-fold bracket of `x` against `y` whichever scalars it is formed over. Restricting scalars along
`ℤ → R` is the case this is used in: a relation `(ad x) ^ n y = 0` proved over `ℤ` is the same
relation over `R`. -/
theorem _root_.LieAlgebra.ad_pow_apply_eq {R S L : Type*} [CommRing R] [CommRing S] [LieRing L]
    [LieAlgebra R L] [LieAlgebra S L] (x : L) (n : ℕ) (y : L) :
    (LieAlgebra.ad R L x ^ n) y = (LieAlgebra.ad S L x ^ n) y := by
