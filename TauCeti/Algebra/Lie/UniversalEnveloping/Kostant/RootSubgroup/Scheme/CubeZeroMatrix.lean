...
* `TauCeti.UniversalEnvelopingAlgebra`
  `.exists_map_genericMatrix_kostantRootSubgroupCoordinateMap_eq_one_add_smul_add_smul`:
  the matrix of a cube-zero root subgroup, `1 + t X + t² Y`, read on the generic matrix along the
  root-subgroup coordinate morphism.
-/
