...
* `TauCeti.GeneralLinear.matrixCoact`: the candidate coaction given by the matrix columns.
* `TauCeti.GeneralLinear.matrixComodule`: the resulting comodule for a grouplike matrix.
* `TauCeti.GeneralLinear.coefficientMatrix_matrixComodule`: its coefficient matrix is the input.
-/
...
hypotheses of `TauCeti.GeneralLinear.matrixComodule`. -/
...
@[simp]
theorem matrixCoact_apply_basisFun (j : Fin n) :
...
noncomputable def matrixComodule : Comodule R S (Fin n → R) where
  ...
  coassoc := by
    ...
    simp only [map_sum, LinearMap.rTensor_tmul, matrixCoact_apply_basisFun,
      ...
      LinearMap.lTensor_tmul, (map_comul_iff R n Y).mp hcomul,
  ...
  lTensor_counit_comp_coact := by
    ...
    simp only [LinearMap.coe_comp, Function.comp_apply]
    rw [Pi.basisFun_apply, matrixCoact_apply_basisFun]
    simp only [map_sum, LinearMap.lTensor_tmul, (map_counit_iff R n Y).mp hcounit]
...
theorem coefficientMatrix_matrixComodule :
  ...
  rw [Comodule.coefficientMatrix_apply, Comodule.matrixCoefficient_def,
    matrixComodule_coact R n Y hcomul hcounit, Pi.basisFun_apply,
