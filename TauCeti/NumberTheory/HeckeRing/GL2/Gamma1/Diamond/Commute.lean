...
`⟨d⟩` and `Tₚ` are both elements of the Hecke algebra `𝕋 Δ₀(N) Γ₁(N) ℤ`. Their commutation
enables simultaneous Hecke/diamond eigenspace decompositions.
...
The criterion is that a diamond representative normalizes `Γ₁(N)` and its conjugation preserves
the double coset of `diag(1, p)`. The latter stability result is available when `p` is prime.
-/
