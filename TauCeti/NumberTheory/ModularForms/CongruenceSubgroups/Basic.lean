...
the divisibility API consumes. To put such a matrix into `Γ₁(N)`, apply the unbundled `mpr`
direction `mem_Gamma1_of_dvd_lowerRow`; to feed a `Γ₁(N)` hypothesis to something
divisibility-shaped, destructure `.mp` of this lemma. -/
