...
module `8ₛ ⊕ 8_c` is not stable under it. That branch is built on the tripled carrier
`TauCeti.D4Tripled.groupScheme` in `TauCeti/GroupTheory/SpecificGroups/CFSG/TrialityD4.lean`; the
spin-carrier points and Frobenius attached here to a triality-twisted index are not the ambient
group and Frobenius of that branch.
