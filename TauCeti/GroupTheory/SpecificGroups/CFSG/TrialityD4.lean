...
theorem graphAut_simpleRootSubgroup (i : Fin d.1.rank) (u : Multiplicative d.1.Closure) :
  ...
  rw [graphAut_def, simpleRootSubgroup_def, simpleRootSubgroup_def,
    D4Tripled.trialityPoints_rootSubgroupPoints, D4Tripled.trialityRootPerm_inl]
  -- On the introduction form of the index the diagram permutation is `trialityPermD4` by
  -- `GraphTwistedIndex.diagramPerm_trialityD4`, and the rank is `4` by computation, so the two
  -- casts are the identity.
  obtain ⟨q, rfl⟩ := d.exists_eq_of
...
theorem graphAut_frobenius (g : d.AmbientGroup) :
  ...
  exact D4Tripled.trialityPoints_frobenius _ _ _ g

/-- The graph automorphism commutes with the Frobenius, as an identity of endomorphisms. This is
the relation `γ₃ ∘ Frob_q = Frob_q ∘ γ₃` required of the graph-twisted families. -/
...
/-- Pointwise, the Steinberg map applies the Frobenius and then triality. -/
